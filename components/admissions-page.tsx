  ...
  return (
    <div lang={chrome.htmlLang}>
      ...
      <main>
        ...
        <section className="adRequirements contentSection"><div className="shell adRequirementsGrid"><Reveal><SectionHeading label={page.requirements.label} title={page.requirements.title} description={page.requirements.description} /></Reveal><div><div className="adRequirementList">{page.requirements.items.map((item, i) => <article key={item.title}><span>0{i + 1}</span><div><h3>{item.title}</h3><p>{item.description}</p></div></article>)}</div><p className="adNote">{page.requirements.note}</p></div></div></section>
