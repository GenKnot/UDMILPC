...
export function WhyMontrealPage({ locale, chrome }: { locale: Locale; chrome: SiteContent }) {
  ...
  return <div lang={chrome.htmlLang}>
    ...
    <main>
      ...
      <section className="wmCta"><div className="shell"><Reveal><p className="sectionLabel">{page.cta.label}</p><h2>{page.cta.title}</h2><p>{page.cta.description}</p><strong className="wmMotto">{page.cta.motto}</strong><div className="wmCtaActions"><Link className="button lightButton" href={`/${locale}/apply`}>{page.cta.actions[0]}<span>↗</span></Link><a className="textLink" href="mailto:info@Montrealic.ca">{page.cta.actions[1]}<span>→</span></a><Link className="textLink" href={`/${locale}/contact`}>{page.cta.actions[2]}<span>→</span></Link></div></Reveal></div></section>
